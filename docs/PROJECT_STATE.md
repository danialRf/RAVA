...
## Manual Toman pricing and simplified product administration

- A product can now be priced directly in Toman by an operator. `product_variants.manual_price_toman` (integer bigint) and `manual_stock_status` carry the price and availability; migration `0006_manual_toman_pricing.sql` is additive and relaxes three constraints only (`quotes.fx_rate_id`, `quotes.fx_toman_per_eur` and `order_items.offer_snapshot` are now nullable). No existing column was dropped and no locked order total was rewritten.
- `manualPrice()` in `@rava/domain` computes the deposit split for an operator-entered price using the same `depositAmount` helper as the estimate engine, so there is no second money implementation. With no pricing rule configured the item is sold pay-in-full. Integer Toman throughout; the split always reconstructs the total exactly.
- `PriceEstimate` now carries `source: "MANUAL" | "SOURCE_OFFER"`. A manual price has `breakdown`, `ruleId` and `rateObservedAt` set to null rather than filled with plausible values, and the storefront states that the price is set by RAVA instead of describing a source observation it does not have.
- A manually priced variant is purchasable with no source offer, no FX rate and no pricing rule. Cart insertion, quote creation and order locking each verify server-side that the variant is active, has a manual price and is in stock; a price the operator changed after quoting fails the lock with `SOURCE_CHANGED`, exactly as a changed source price does.
- Manually priced order lines create no German purchase task and carry a null `offer_snapshot` and null `maxSourcePriceEurCents`. The fail-closed rule for source-priced lines is unchanged.
- The product form now takes price and availability alongside name, brand, category and image, and creates the default variant with them. Adding a sellable product is one form on one page; the German sourcing path is unchanged and remains available under advanced settings for products that should be priced automatically.
- Fixed: the products page `+ افزودن محصول` control was an anchor to a collapsed `<details>` element, so clicking it scrolled to the form without opening it. Button and panel now share one piece of state (`AdminCreatePanel`). This was found by driving the real form in a browser, not by reading the markup.
- Fixed: the E2E runner parsed the database URL from the whole of stdout, so any driver notice on that stream aborted the browser suite before it started.
- Admin navigation now leads with orders, products, payments and customers; sourcing, retailers, automatic pricing, service health and activity history moved under advanced settings. No permission or RBAC rule changed.
- Verified 2026-09-12 against a real PostgreSQL instance and a production build: Prettier clean, ESLint clean, TypeScript clean across all nine packages, 97 unit tests, 83 database integration tests (including seven new manual-pricing cases) and 24 Playwright browser tests including a new end-to-end admin test that creates, prices and publishes a product. Desktop and 390px mobile renderings were inspected visually.
