...
const ORDER_TRANSITIONS: TransitionMap<OrderStatus> = {
  ...
  TRIP_ASSIGNED: ["IN_TRANSIT_TO_IRAN", "TRIP_PENDING"],
  IN_TRANSIT_TO_IRAN: ["ARRIVED_IRAN"],
  ARRIVED_IRAN: ["BALANCE_DUE"],
  BALANCE_DUE: ["BALANCE_PAID"],
  BALANCE_PAID: ["LOCAL_DELIVERY_PENDING"],
  LOCAL_DELIVERY_PENDING: ["OUT_FOR_DELIVERY"],
  // A failed delivery attempt returns to the queue rather than ending the order.
  OUT_FOR_DELIVERY: ["DELIVERED", "LOCAL_DELIVERY_PENDING"],
  DELIVERED: [],
  CANCELLED: [],
